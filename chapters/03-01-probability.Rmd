...
$\frac{k}{N}$. (If in doubt, execute this experiment. By hand or by computer. See below.) This
...
The following code demonstrates how the proportion of black balls drawn from an urn with $k = 7$ black balls and $10$ balls in total, gravitates to the probability 0.7 when we keep drawing and drawing.
...
# take a million samples from the urn (with replacement)
draws <- sample(
  # vector to sample from (default probability is uniform)
  x = urn, 
  # take a million samples
  size = n_samples,
  # put balls back after drawing
  replace = TRUE
)
...
if it comes up tails, we draw from urn 2. The process is visualized in Figure XYZ below.
