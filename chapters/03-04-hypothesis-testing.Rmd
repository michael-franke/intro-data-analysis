...
$$
\begin{aligned}
H_0 \colon \theta_0 = 0.5 && H_a \colon \theta_0 \neq 0.5
\end{aligned}
$$
...
But in the case of testing whether there is a bias towards heads ($\theta > 0.5$), the null hypothesis is the same but the alternative hypothesis is one-sided:

$$
\begin{aligned}
H_0 \colon \theta_0 = 0.5 && H_a \colon \theta_0 < 0.5
\end{aligned}
$$
...
Though central to much of frequentist statistics, $p$-values are frequently misinterpreted, even by seasoned scientists [@HallerKrauss2002:Misinterpretati]. To repeat, the $p$-value measure the probability of observing, if the null-hypothesis is correct, a value of the test statistic that is (in a specific, contextually specified sense) more extreme than the value of the test statistic that we assign to the observed data. We can therefore treat $p$-values as a measure of evidence *against* the null-hypothesis. And if we want to be even more precise, we interpret this as evidence against the whole assumed data-generating process, a central part of which is the null-hypothesis.
...
$$ p(D) > \alpha \ \ \text{iff} \ \ \theta_0 \in \text{CI}(D) $$
where $\text{CI}(D)$ is the $(1-\alpha) 100\%$ confidence interval constructed for data $D$.
...
### Pearson's $\chi^2$-test {#ch-03-05-hypothesis-testing-Pearsons-Chi}
