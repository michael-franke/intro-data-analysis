...
After seeing a frequentist model and learning about test statistic and sampling distribution, let's explore what a $p$-value is based on the frequentist Binomial Model. Our running example with be the 24/7 case, where $N = 24$ and $k = 7$. Notice that we are glossing over the "raw" data immediately and work with the value of the test statistic of the observed data directly: $t(D_{\text{obs}}) = 7$).

Remember that the mathematical definition given above aims to capture the idea that $p(D_{\text{obs}})$ is the probability of observing any alternative outcome with a value of the test statistic that is at least as extreme evidence against $H_0$ as $t(D_{\text{obs}})$:

$$
  p(D_{\text{obs}}) = P(T^{|H_0} \succeq^{H_0} t(D_{\text{obs}})) % = P(\mathcal{D}^{|H_0} \in \{D \mid t(D) \ge t(D_{\text{obs}})\}) 
$$

To fill this with life, we need to set a null hypothesis, i.e., a value of coin bias $\theta$ that we would like to collect evidence *against*. A fixed $H_0$ will directly fix $T^{|H_0}$ but we will have to put extra thought into how to conceptualize $\succeq^{H_0}$ for any given $H_0$. To make exactly this clearer is the job of this section.
...
- Is the coin fair: $\theta \approx 0.5$?
- Is the coin biased towards heads: $\theta > 0.5$?
- Is the coin biased towards tails: $\theta < 0.5$?
- Is the coin's bias between 0.3 and 0.4: $\theta \in [0.3; 0.4]$? 
...
The null-hypothesis fixes a likelihood function for any kind of data we might perceive on (hypothetical) repetitions of an experiment. We here write $P(\mathcal{D} = D \mid H_0)$ or, for short, $P(D \mid H_0)$, e.g., $P(\mathcal{D} = D \mid \Theta = 0.5)$ for the binomial 24/7 example. Let's write $\mathcal{D}^{|H_0}$ for the random variable that captures the probability distribution over possible data observations, on the assumption that the null-hypothesis is true. For example, when fixing $N=24$ the space of logically possible outcomes is $K = \set{0, 1, \dots, 24}$ observations of heads. 
Then $T^{|H_0} = t(D^{|H_0})$ is a derived random variable, the so-called **sampling distribution**, with the function $t \colon d \mapsto t(d) \in \mathbb{R}$ a so-called **test statistic**. 
