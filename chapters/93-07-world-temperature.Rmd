...
Here is a scatterplot of annual average temperature `avg_tmp` against `year`. The straight line is the best linear predictor.
...
The estimated coefficient for variable `year` implies that we should expect an annual increase of average world temperature of about `posterior_samples(fit_avg_temp) %>% 
  pull(b_year) %>% summarize_sample_vector() %>% pull(mean) %>% round(3)` degrees Celcius.