...
This chapter lays out the learning goals of this book (Section \@ref(Chap-01-00-intro-learning-goals)) and describes how these goals are to be achieved (Section \@ref(Chap-01-00-intro-course-structure)). Sections \@ref(Chap-01-00-intro-tools) and \@ref(Chap-01-00-intro-topics) detail which technical tools and methods are covered in the course, and which are not. There will be some information on the kinds of data sets we will use during the course in Section \@ref(Chap-01-00-intro-data-sets). Finally, Section \@ref(Chap-01-00-intro-installation) provides information about how to install the necessary tools for this course.
...
+ **hypothesis testing**: different ways of testing scientific hypothesis

+ **generalized regression**: how to apply generalized regression models to different types of data sets

There is, obviously, a lot that we will *not* cover in this course. We will, for instance, not dwell at any length on the specifics of algorithms for computing statistical inferences or model fits. We will also only deal with the history and the philosophy of science of statistics at the very end of this course and only to the extent that it helps us to better understand the theoretical notions and practical habits that are important in the context of this course. We will also not do extremely heavy math.

Data analysis can be quite varied because data itself can be quite varied. We try to present some variation, but since this is an introductory course with lots of other ground to cover, we will be slightly conservative in the kind of data that we analyze. We will discuss data from behavioral experiments (like categorical choices or reaction times).
But there will be no analyses specifically tailored to pictures, sounds, dates or time sequences in this course.

There are at least two different motivations for data analysis, and it is important to keep them apart. This course focuses on **data analysis for explanation**, i.e., routines that help us understand reality through the inspection and massaging of empirical data. We will only glance at the alternative approach, which is **data analysis for prediction**, i.e., using models to predict future observations, as commonly practiced in machine learning and its applications. In sloppy slogan form, this course treats data science for scientific knowledge gain, not as an engineer's application.
...
Note: to open the installation package you have to press Control + right click and then choose "open". You also may need to register for an Apple Developer account (free of charge).
...
*Next steps are required to install RStan*

Execute in the RStudio console:

`install.packages("Rcpp", repos = "https://rcppcore.github.io/drat")` 

Optimize the C++ compiler:
...
`Sys.setenv(MAKEFLAGS = "-j4")` # set desired number of cores to use

`install.packages("rstan", type = "source")`

Check if Stan installation was successful by running the example model:
...
Check if installation was successful by running the example model:
