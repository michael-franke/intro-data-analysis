...
1. **Prediction**: Models can also be used to make predictions about future or hypothetical data observations. We will see an example of this is Section \@ref(Chap-03-03-models-parameters-priors) in this chapter. 
...
Recall that *Binomial Model* has a binomial likelihood function:
...
When models get very complex and incorporate many parameters it can be difficult to tease out all relations between the model components. 
In such a situation a graphical notation of a model might be helpful. 
...
In the graphical notation we introduced in Section \@ref(Chap-03-03-models-representation), parameters usually show up as white nodes, because they are unknowns.
...
tibble(
  x = seq(from = 0, to = 24),
  `point-valued` = dbbinom(x, size = trials, alpha = 50000, beta = 20000),
