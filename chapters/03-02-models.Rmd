...
tibble(
  ...
  uninformative = dbbinom(x, size = trials, alpha = 0.5, beta = 0.5),
...
table_data <- tribble(
  ...
  "inference", "$\\theta$",  "MLE: $\\hat{\\theta} = \\arg \\max_{\\theta}\  P_M(D \\mid \\theta)$", "posterior: $P_M(\\theta \\mid D)$",
...
Towards a model that computes these linear regression lines, one for each group, that is in line with the idea introduced above that, for the purposes of inference and testing, we might like to represent differences between groups as $\delta$ parameters in a model, let's assume that the type of advocado is our grouping variable $g_i$. The default group (say: conventional avocados) has $g_i = 0$, the other group has $g_i = 1$. The first group gets "its own" intercept $\beta_0$ and slope $\beta_1$. The second group gets additive offsets $\delta_0$ and $\delta_1$ for its slope and intercept, respectively. The full model is shown in Figure \@ref(fig:ch-03-02-Linear-Regression-Groups).
