...
<!--
Ex. Calculate the probability of observing no more than 7 successes for $N= 24$ and $\theta =0.2$.
...
<!--
Ex. Write the described likelihood function in R.  It should take two vectors and free parameters: \mu, \sigma, \delta(the difference between the means))
```{r}
...
<!---
Ex. You want to check if a coin is fair. You toss it for 100 times and 22 times get heads. Now you can compute likelyhood that p==0.5 is accurate for it. 
#40 tosses
...
You want to run a Bayesian model to calculate how many pizzas to order for a party. Which prior would You use if:
    + only close friends are coming and you know their habits: **strongly informative**
    + not that close friends are coming and you have a rough idea of their habits: **weakly informative**
    + you invited new acquaintances  **uninformative prior**s
...
If we flip a coin once, the likelihood of each outcome $x \in X = \{0;1\}$ (representing heads or tails, encoded as 1 and 0, respectively) can be modeled with the **Bernoulli distribution** as follows, where $\theta \in [0;1]$ is the coins bias towards landing heads:
...
The Flip-and-Draw-Hypergeometric Model is exactly like the previous Flip-and-Draw Model, except that we allow ourselves to sample repeatedly *without replacement* from an urn the coin flip selected for us. The probability of observing $k$ black balls when drawing $n$ balls from an the which contains $N$ balls in total out of which $K$ balls are black when we do not put each drawn ball back into the urn is described the so-called [hypergeometric distribution](https://en.wikipedia.org/wiki/Hypergeometric_distribution). The hypergeometric distribution assign to each $k \in \{\max(0, n+K-N), \dots, \min(n,K)\}$ the probability mass:
...
+ You want to find out how many people visited your webpage for the first time.  You know that overall  50 people were there. And you see 7 comments, five of which are left by your old followers.  
+ if running speed increases with every 100 m among sprinters, but not within marathoners.
+ relationship between time spent on studying and the amount of coffee consumed per day.
+ if a football referee biased and gives yellow cards more often to one of the teems. Consider that the fold probability is known for each team.
