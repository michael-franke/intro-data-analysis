...
<pre class="webppl">
viz(repeat(1000, function(x) {beta({a: 4, b: 6})}));
</pre>
...
Here's an example of how to perform inference using the MCMC method. Click on the yellowish box to check what the code does and how `Infer()` is used. Please re-visit Chapter \@ref(ch-03-03-MCMC) for more information on how the MCMC algorithm roughly works.
...
- Press `Ctrl` + `/` to comment or uncomment code (apparently, only works with an English keyboard).
