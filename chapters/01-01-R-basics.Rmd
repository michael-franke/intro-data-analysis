...
R also provides functional iterators (e.g., `apply`), but we will use the functional iterators from the `purrr` package. Here is a first example of how this code looks in a functional style using the functional iterator `map_dbl` from `purrr`:
...
Piping let's you pass the result of a previous function call into the next:
