...
#### Pipes in the `magrittr` package

The base pipe operator `%>%` is automatically imported from the `magrittr` package when you load the `tidyverse` package.
The `magrittr` package has three more useful pipe operators, which are /not/ loaded automatically when you load the `tidyverse` package.

The **tree pipe** `%T>%` passes on to the RHS whatever it was fed on the LHS. 
...
The **exposition pipe** `%$%` is like the base pipe `%>%` but makes the names (e.g., columns in a data frame) in the LHS available on the RHS, even when the function on the RHS normally does not allow for this.
...
Finally, the **assignment pipe** `%<>%` which pipes the LHS into a chain of computations and assigns the final value back to the LHS.
...
### Excursion Multiple assignments (or: "unpacking") with the `zeallot` package
