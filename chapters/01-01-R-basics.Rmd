...
Notice that the result is a matrix with a single column. This is important. R uses so-called *column-major mode*.^[Python, on the other hand, uses the reverse *row-major mode*.] This means that it will fill columns first. For example, a matrix with three columns based on a six-placed vector , 2, \dots,  will be built by filling the first column from top to bottom, then the second column top to bottom, and so on.^[It is in this sense that the "first index moves fastest" in column-major mode, which is another frequently given explanation of column-major mode.]
...
Create a function called `bigger_100` which takes two numbers as input and outputs 0 if their product is less than 100, and 1 otherwise. (**Hint:** remember that you can cast a Boolean value to an integer with `as.integer`.) 
...
We can write this even shorter, using `purrr`'s short-hand notation for functions:^[Just for the record, we can achieve the same result also by `ifelse(input_vector %% 2, input_vector * 10, input_vector)`.]
...
Rmarkdown is a simple markup language which embeds R code and its output in order to produce a large variety of document types as output, including HTML or PDF (see Figure \@ref(fig:Rmarkdown-cartoon)).

```{r Rmarkdown-cartoon, echo = FALSE, fig.cap = "Artwork by @allison_horst", out.width=300}
knitr::include_graphics("visuals/allison_horst/rmarkdown_wizards.png") 
```

A strong argument for using something like Rmarkdown in your research pipeline in **reproducibility** (see Figure \@ref(fig:reproducibility-cartoon)). When you prepare a data set and perform a statistical analysis, for example, you will by necessity make a lot of detailed decisions, not all of which are easy to report in natural language, but all of which combine contribute to the results you eventually report, e.g., in a research or term paper. Since science aims for objectivity and social structures of scrutinizing previous results, it is important to share with other researcher *all* of your assumptions and decisions of detail. Ideally, therefore, you can share something like a nicely commented Rmarkdown file which contains every step of the analysis. 

```{r reproducibility-cartoon, echo = FALSE, fig.cap = "Artwork by @allison_horst", out.width=300}
