...
Create a vector `a` that contains the names of three of your best friends and a vector `b` with their age. Create a data frame that represents this information (One column with names and one with respective age). Notice that column names should represent the information they contain!
...
Print out only the names of friends from the `best_friends` data frame.
...
Print out only the names of friends who are younger than 22.
...
Create a vector `a` which contains the names of three of your best friends and a vector `b` with their age. Create a tibble with two columns: name, age. Print it. Now, add a new column named "birth date" and the respective values. Add another column, which states `TRUE`, if the date of birth is even. HINT: you can create a new column dynamically! First, you have to declare a column on the left sight (tibble["name-of-the-column"]), and then assign a value to it. For the latter you might want to check out the modulo operator (%%).
<div class="collapsibleSolution">
<button class="trigger">Solution</button>
<div class="content">
```{r}
...
best_friends["birth date"] = c(3, 5, 6)
best_friends["is even"] = best_friends["birth date"] %% 2 == 0
best_friends
