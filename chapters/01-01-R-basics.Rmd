...
Create two variables, `a` and `b`, and assign values 103 and 14 to them, respectively. Next, divide the first one by the second and produce an output with 3 digits after comma.
...
b <- 14    # assign value 103 to variable `b`
...
Create a vector which contains all even numbers from 0 to 20 and assign it to a variable.  Now transform the variable such that it contains only odd numbers up to 20 using mathematical operation. Notice that the numbers above 20 should not be included! HINT: you may use indexing to fulfill these criteria.
...
Create a matrix with three columns and three rows that contain random integers from 1 to 100 without replacement. HINT! You may first produce an array with random integers and then parse it to the matrix() function
...
Create a vector A with 10 random integers in range (1:50) and the other. Then, using for-loop make the second, filled with the mean of the corresponding integer in vector A, one integer above it, and one after (For first and last integer missing values are equal to 0). Print out the result as a tibble. 
