...
sample_Bayes_noninform_regression <- function(
  X, # design matrix
...
samples_Bayes_regression <- sample_Bayes_noninform_regression(X,y, 10000)
...
Since the 95% credible interval clearlyl excludes this value, we might (based on this binary decision logic) now say that based on the model and the data there is reason to believe that `unemployment` *does* contribute to predicting `murder_rate`, in fact that the higher a city's unemployment rate, the higher we should expect its murder rate to be.
(Remember: this is not a claim about any kind of causal relation!)

## Comparison of approaches

We saw three conceptually different approaches to linear regression: based on ordinary least squares, based on likelihood alone and based on Bayesian inference (with non-informative priors).
At the core of linear regression lies the linear predictor $\xi = X \beta$, which all three approaches used.
The different approaches diverge in how they determine the regression coefficients that feed this linear predictor.
