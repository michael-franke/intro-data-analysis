...
The prediction corresponding to the best parameter values is denoted by $\hat{\xi}$ and called the *best linear predictor*:
...
The design matrix includes the values for all predictor variables and it also includes an "intercept column" $X_0$ for which $X_{i0}=1$ for all $1 \le i \ne n$ so that the intercept $\beta_0$ can be treated on a par with the other regression coefficients.
...
Using the design matrix $X$, the linear predictor vector $\xi$ is:
...
How can we interpret the parameters $a$ and $b$ of the linear model $\xi_i = a x_i + b$ have? How are they usually called in regression jargon?
...
Parameter $a$ gives the amount of change of $y$ for each unit of change in $x$. 
Parameter $b$ gives the value of $y$ at $x=0$.
...
In the above example, where we regressed `murder_rate` against `unemployment`.
The model has two regression coefficients: an intercept term and a slope for `unemployment`.
...
We can use the `optim` function to find the best-fitting parameter values for our simple linear regression.
