...
So, here is a function call of `brms:brm` which (i) specifies a reasonably unconstrained but proper parameter for the slope coefficient for `year` and (ii) also collects samples from the prior (by setting the option `sample_prior = TRUE`).
...
The table shows the estimate for the slope of `year`, together with an estimated error, lower and upper bounds of a credible interval (95% per default).
