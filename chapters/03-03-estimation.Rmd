...
Given a Bayesian model $M$, we can use Bayes rule to update prior beliefs about $\theta$ to obtain so-called _posterior beliefs_ $P_M(\theta \mid D)$, which represent the new beliefs after observing D and updating in a conservative, rational manner based on the assumptions spelled out in $M$, which represent the new beliefs after observing D and updating in a conservative, rational manner based on the assumptions spelled out in $M$.
We will see two different methods of computing posterior distributions $P_M(\theta \mid D)$, a precise mathematical derivation with limited applicability in terms of so-called _conjugate priors_, and an approximate, sampling-based method based on so-called _Markov Chain Monte Carlo algorithms_.
...
Fix a Bayesian model $M$ with likelihood $P(D \mid \theta)$ for observed data $D$ and prior over parameters $P(\theta)$. We then update our prior beliefs $P(\theta)$ to obtained posterior beliefs by Bayes rule:^[Since parameter estimation is only about one model, it is harmless to omit the index $M$ in the probability notation.]
...
The shorthand notation for the posterior, repeated below, makes it particularly clear that the posterior distribution is a "mix" of prior and likelihood:

$$\underbrace{P(\theta \, | \, D)}_{posterior} \propto \underbrace{P(\theta)}_{prior} \ \underbrace{P(D \, | \, \theta)}_{likelihood}$$

Let's explore this first before worrying about how to compute posteriors concretely.
...
Combining the four different priors and the two different data sets, we see that the posterior is indeed a mix of prior and likelihood. In particular, we see that the weakly informative prior has only little effect if there are many data points (the KoF data).
...
b. Based on the notation above, which of the following statements do you think is true?
...
We will introduce both below, alongside some alternative estimates (namely the _maximum a posteriori_ and the _maximum likelihood estimate_).
...
### Excursion: Computing point-estimates in R
...
Use the function `aida::summarize_sample_vector()`.
