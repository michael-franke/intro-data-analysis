...
The chapter also introduces common **point-valued and interval-ranged estimates** for parameters, in particular the Baysian measures of **posterior mean** and **credible intervals**.
...
```{r ch-03-03-estimation-normal-uninformative-model, echo = F, fig.align= "center", fig.cap="A model to infer the parameter of a normal distribution with non-informative priors."}
...
Here is a convenience function that takes a vector `data_vector` (like `height_A`) of metric observations as input and returns `n_samples` samples from the posterior.
...
```{r}
get_samples_single_noninformative(heights_A, n_samples = 5)
```
...
post_samples_A_noninfo <- get_samples_single_noninformative(data_vector = heights_A, n_samples = 10000)
...
The model with uninformative priors is useful when modellers have no or wish to not include any *a priori* assumptions about $\mu$ and $\sigma$.
...
```{r ch-03-03-estimation-normal-conjugate, echo = F, fig.align = "center", fig.cap="Model with conjugate priors."}
...
post_samples_A_conjugate_2 <- get_samples_single_normal_conjugate(
  heights_A,
  nu    = 1,
  var   = 1/1000,
  mu    = 40,
...
rbind(
  ...
  summarize_sample_vector(post_samples_A_conjugate_2$mu,    "mu") %>% mutate(model = 2),
  summarize_sample_vector(post_samples_A_conjugate_2$sigma, "sigma") %>% mutate(model = 1)
)

```

The posterior is a mixture of prior and likelihood.
The prior for model 1 is rather weak (high variance, low $\kappa$ leading to large range of plausible values for $\mu$).
...
If the derived samples of $\delta$ are credible bigger than zero, there is reason to believe that there is a difference between flower types such that 'B' is bigger than 'A'.
