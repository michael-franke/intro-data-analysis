...
To see this, notice that the impact of polite contexts on pitch for female speakers is:

$$
\text{eff_context_on_female} = (\beta_0 + \beta_\text{pol}) - \beta_0 = \beta_\text{pol}
$$
The effect of polite contexts on male speaker's pitch is:

$$
\text{eff_context_on_male} = (\beta_\text{pol} + \beta_\text{male}) - (\beta_0 + \beta_\text{male} + \beta_{\text{pol}} + \beta_\text{pol&male}) = \beta_{\text{pol}} + \beta_\text{pol&male}
$$

Therefore, the difference -comparing male and female speakers0- of the impact of the context manipulation (going from informal to polite contexts) is:
$$\text{eff_context_on_male} - \text{eff_context_on_female} = \beta_\text{pol&male}$$

How do these model coefficients help address the research hypotheses we formulated above? - 
The interaction term directly encodes what we had formulated as hypothesis 3 above.
In other words, we can express H3 as the parameter-based hypothesis that:

$$\textbf{H3:} \ \ \ \ \beta_\text{pol&male} < 0$$
...
$$
\textbf{H1:} \ \ \ \ 2 \beta_\text{male} + \beta_\text{pol&male} < 0
$$

Similar reasoning leads to the following formulation of hypothesis H2 concerning a main effect of factor gender:

$$
\textbf{H2:} \ \ \ \ 2 \beta_\text{pol} + \beta_\text{pol&male} < 0
$$
...
We can then use the function `brms::hypothesis` to test the hypotheses in question.
