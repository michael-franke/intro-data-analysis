...
d_sum_stats_blocks_cleaned = d_cleaned %>% 
  group_by(block) %>% 
  summarise(
    HDI_low  = mean(RT) - sd(RT),
    mean_RT  = mean(RT),
    HDI_high = mean(RT) + sd(RT)
  )
...
```{r}
d_sum_stats_blocks_cleaned %>% 
  ggplot(aes(x = block, y = mean_RT, fill = block)) +
  geom_col() +
  geom_errorbar(aes(ymin = HDI_low, ymax = HDI_high), size = 0.3, width = 0.4 ) +
...
##### Working-class differences in Germany
