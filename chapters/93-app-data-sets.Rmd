...
bootstrapped_CI <-  function(data_vector, n_resamples = 1000) {
  ...
  tibble(
    'lower' = quantile(resampled_means, 0.025) %>% as.numeric(),
    'upper' = quantile(resampled_means, 0.975) %>% as.numeric()
  ) 
}
...
d_sum_stats_blocks_cleaned <- d_cleaned %>%
  group_by(block) %>%
  summarise(
    lower = bootstrapped_CI(RT)$lower,
    mean_RT  = mean(RT),
    upper = bootstrapped_CI(RT)$upper
  )
