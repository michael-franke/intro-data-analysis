...
We want to infer a propability distribution given certain constraints. For example a coin flip experiment can have only a dichotomous outcome {0,1} and has a constant probabiliy. Which distribution should be used in order to model this scenario?
...
Consequently, the question arises how "ignorance" can be measured and distributions compared according to their "information content"? This will be topic of the upcoming exursos, the key word here comes from information theory and is "Entropy" in connection with the "Maximum Entropy Principal". Which establishes also the connection between exponential family and maximum ignorance distributions. In a nutshell, the maximum entropy principal starts with constraints that are imposed on a distribution and derives by maximizing entropy a probability density/mass function. And finally, distributions belonging to the exponential family arise as solutions to the maximum entropy problem subject to linear constraints.
...
### Information Entropy
*Entropy* is a measure of the information content of an outcome of $X$ such that less probable outcomes convey more information than more probable ones. When the goal is to find a distrbution that is as ignorant as possible, then, consequently, the entropy should be maximal. Entropy is defined as follows:
...
We can refine this procedure by using smaller quanta, i.e. large $n$. 

When $n \rightarrow \infty$, then $n_i \rightarrow \infty$ in such a way that $\frac{n_i}{n} \rightarrow p_i = \text{const.}$,
...
An **overview of Maximum Entropy distributions** can be found on [Wikipedia](https://en.wikipedia.org/wiki/Maximum_entropy_probability_distribution), here are some further examples:
 ...
 [0,1]          | $E(\ln(x))=\psi(\alpha)-\psi(\alpha+\beta)$  |$f(x)=\frac{x^{\alpha-1}(1-x)^{\beta-1}}{B(\alpha,\beta)}$                                                                      | Beta
${}$ | $E(\ln(1-x))=\psi(\beta)-\psi(\alpha+\beta)$ |  |  
[0,$\infty$)    | $E(x)=k, E(\ln(x))=\psi (\frac{k}{2})+ \ln(2)$ | $f(x)=\frac{1}{2^{k/2}\Gamma(k/2)}x^{\frac{k}{2}-1} \exp(-\frac{x}{2})$  |  Chi-squared
 $\mathbb{N} \cup \{0\}$ | $E(x)=\lambda$, f $\in$ $\infty$-generalized binomial distribution | $f(k)=\frac{\lambda ^k \exp(- \lambda)}{k!}$ | Poisson
