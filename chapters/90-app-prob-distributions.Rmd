...
```{r ch-app-01-normal-distribution-cumulative, fig.cap = "Examples of the cumulative normal distribution corresponding to the previous probability density functions."}
ggplot(rv_normal_intro, aes(x =x, color = parameter_set)) +
...
The normal distribution is often associated with the \emph{68-95-99.7 rule}. It shows the probability of a random data point landing within \emph{one}, \emph{two} or \emph{three} standard deviations of the mean (see Fig.~\ref{fig:ch-app-01-normal-distribution-coverage} depicting the three intervals). For example, about 68% of values drawn from a normal distribution are within one standard deviation $\sigma$ away from the mean $\mu$.
...
```{r ch-app-01-chi-squared-distribution-density, fig.cap = "Examples of chi-squared probability density distributions."}
# how many samples to use
n <- 1e6
...
ggplot(rv_chisq_intro, aes(x = x, color = parameter_set)) +
  ...
  stat_density(geom = "line", position="identity", size = 2) +
  # add text annotations to lines
  annotate("text", x=17, y=0.15,  label="X~Chi-Squared(3)", color=project_colors[1], size = 6) +
  annotate("text", x=30 , y=0.09,  label="X~Chi-Squared(10)",  color=project_colors[2], size = 6) +
  annotate("text", x=40,   y=0.05, label="X~Chi-Squared(20)",  color=project_colors[3], size = 6) +
  # suppress legend
  theme(legend.position = "none")
```
...
```{r vch-app-01-F-distribution-density, fig.cap = "Examples of F probability density distributions."}
# how many samples to use
n <- 1e6

rv_F_intro <- tibble(
  X = rf(n, df1 = 10, df2 = 10),
  Y = rf(n, df1 = 20, df2 = 20),
  Z = rf(n, df1 = 30, df2 = 30)
) %>% 
  pivot_longer(cols = c("X", "Y", "Z"), names_to = "parameter_set", values_to = "x")
...
ggplot(rv_F_intro, aes(x = x, color = parameter_set)) +
  ...
  stat_density(geom = "line", position="identity", size = 2) +
  # add text annotations to lines
  annotate("text", x=10, y=1.25,  label="X~F(10,10)", color=project_colors[1], size = 6) +
  annotate("text", x=10 , y=1,  label="X~F(20,20)",  color=project_colors[2], size = 6) +
  annotate("text", x=10,   y=0.75, label="X~F(30,30)",  color=project_colors[3], size = 6) +
  # suppress legend
  theme(legend.position = "none")
```
...
ggplot(rv_student_intro, aes(x = x, color = parameter_set)) +
  # draw density lines
  stat_density(geom = "line", position="identity", size = 2) +
  # add text annotations to lines
  annotate("text", x=10, y=0.15,  label="X~t(3)", color=project_colors[1], size = 6) +
  annotate("text", x=10 , y=0.09,  label="X~t(15)",  color=project_colors[2], size = 6) +
  annotate("text", x=10,   y=0.05, label="X~t(30)",  color=project_colors[3], size = 6) +
  # suppress legend
  theme(legend.position = "none") 
