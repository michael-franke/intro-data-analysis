...
Seeing this difference also explains why sometimes frequentist and Bayesian approaches given different results when testing the same null-hypothesis based on the same observed data.
...
In contrast, using the Savage-Dickey method to compute the Bayes factor, we find strong supprt *in favor of* $H_0$.

```{r}
dbeta(0.5, k+1, N - k + 1)
```  

The reason why these methods give to different results is because they *are* conceptually completely different things. There is no genuine paradox.
...
The agile data analyst will diligently check each concrete research context for which method is most conducive of gaining the insights relevant for the given purpose.

The next section also summarizes these different approaches to hypothesis testing.

## Explicit beliefs vs. impplicit intentions

The main objective against Bayesianism which motivated and still drives the frequentist program is that Bayesian priors over parameters and models are *subjective*, and therefore to be regarded less unscientific than hard *objectively justifiable* ingredients.
...
In contrast, the frequentist notion of a $p$-values (and with it the confidence interval) relies on something *even more* mystic, namely the researcher's intentions during the data collection, something that is no even in principle openly scrutinizable after the fact. 
To see how central frequentist notions rely on implicit intentions and counterfactual assumptions about data we could have seen but didn't, let's consider an example (see also [@Wagenmakers2007:A-practical-sol,@kruschke2015] for discussion).
...
The manner of data collection dictates what other possible obervations of the experiment are.
...
$$ 
\text{Binomial}(k ; n = 24, \theta = 0.5) = \choose{n}{k} \theta^{k} \, (1-\theta)^{n-k} 
$$
...
But we we look at the "Stop at $k=7$" case, we need a different likelihood function.
...
The researchers intentions about how to collect data influence the statistical analysis.
...
To see that normalizing constants cancel out for parameter estimation, see:
