...
Conceptual considerations about what we may or should attach probabilities to aside, the most obvious consequence of this is that Bayesian approaches (i) are more complex to compute or analyze, and (ii) provide richer information.
But there are more subtle differences, and so this chapter will explore some of these in order to also contribute a better understanding of both approaches in isolation.



- explicit beliefs vs. implicit intentions

- error control & baked-in decision rules

- Bayesian power


## Comparing the information provided by each account

- intuitively interpretable numbers
...
Consider the well-known 24/7 case again.
...
Testing a null hypothesis of no bias $\theta_c = 0.5$ based on this data with R's built-in Binomial test we get:

```{r }
binom.test(k_obs, n_obs)
```

Here is how we can also use Monte Carlo simulation to approximate this $p$-value:

```{r}
# number of MC samples
x_reps = 500000

# likelihood of each sampled data 
lhs = map_dbl(1:x_reps, function(i) {
  # sample a hypothetical data point
  k_hyp = rbinom(1, size = n_obs, prob = 0.5)
  # determine how likely this hypothetical observation is
  dbinom(k_hyp, size = n_obs, prob = 0.5)
})

# how likely is the observed data (under H0)
lh_obs = dbinom(k_obs, size = n_obs, prob = 0.5)

# proportion of samples that are "at least as extreme"
...
Notice that there might be different notions of what counts as "best" in place. Still, the frequentist "best" estimate seems rather extreme.

What about interval-ranged estimates? 
Which is the better tool, confidence intervals or credible intervals? 
-- This is hard to answer. 
Numerical simulations can help answer these questions.^[Even where the math seems daunting, simulation methods are much more tangible and applicable and sometimes require only basic programming experience.] 
...
To do so, we repeatedly sample a ground-truth (e.g., a known coin bias $\theta_{\text{true}}$) from a flat distribution over $[0;1]$.^[This is already not innocuous. We are fixing, as it were, an assumption about how likely ground-truths should actually occur in the real world.]. 
