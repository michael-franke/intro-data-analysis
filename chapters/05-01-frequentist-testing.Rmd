...
Bayesian ideas have been given a solid mathematical treatment starting in the 18th century by French mathematician Pierre-Simon Laplace.
A rigorous philosophical underpinning of subjective probability has been given in the early 20th century.
Still, the rise of statistics as a modern, ubiquitous tool for empirical knowledge gain in science took off in the first part of the 20th century in a distinctly non-Bayesian direction.
...
2. Bayesian inference is very hard (mathematically and computationally).^[Notice also that Bayesian computation is a very recent field. The MCMC algorithm was only invented in the middle of the 20th century. Before close to the turn of the 21st century, researchers lacked wide-spread access to powerful computers capable of running algorithms that approximate Bayesian inference for complex models.]

As an alternative to Bayesian methods, the dominant method of statistical inference of the 20th century is **frequentist statistics**.
...
A crude (and certainly simplified) explanation of why frequentist approaches eshew subjective beliefs is this. 
...
The only statements about probabilities that are conceptually sound, according to a fundamentalist frequentist interpretation, are those that derive from intuitions about limiting frequencies when (hypothetically) performing a random process (like throwing a dice or drawing a ball from an urn).
...
As a result, the frequentist approach to statistical inference needs alternative methods for parameter estimation, inference and model comparison --- methods that do \emph{not} rely on (subjective) priors over parameters or models.
...
Central to frequentist statistics is what is often called **null-hypothesis significance testing**. 
The general idea (again: simplified) is this.
Say, we are interested in the question of whether a given coin is fair, so: $\theta_c = 0.5$.
We refuse to express beliefs about the likely value of $\theta_c$, neither prior nor posterior.
...
The null models assumes that $\theta_c = 0.5$ but also uses the obvious likelihood function, $\text{Binomial}(k, N, \theta_c = 0.5)$, stating how likely it would be to observe $k$ heads in $N$ tosses, for fixed $\theta_c$.
These probabilities are non-subjective, but grounded in frequentist justification about limiting frequencies of events.
...
If the agent's perplexity is extremely high, that is if the null-model would not have predicted the data at hand to a high enough degree, we take this as **evidence against the null-model**.
...
Historically, there have been different schools of thought in frequentist statistics, all of which relate in some way or other to a quantified notion of evidence against a null-model, but that differ substantially in other ways.^[There is no universally accepted Bayesian treatment either. This is why it is important to apply rational deliberation in each case, and not to succumb to the temptation of following easy recipes too quickly.]
The three main approaches, shown in Figure \@ref(fig:ch-03-04-frequenist-flavors), are Fisher's approach, the Neyman-Pearson approach and the (modern) hybrid approach, often referred to simply as NHST (= null hypothesis significance testing).
In simplified terms, the main difference is that Fisher's approach is epistemic (relating to beliefs), while the Neyman-Pearson approach is alethic (relating to actions).
Fisher's approach calculates $p$-values of measures of evidence against a model, but does not necessarily prescribe what to do this this quantitative measure.
The approach championed by Neyman-Pearson calculates a $p$-value and also fixes categorical decision rules relating to whether to accept or reject the null-model (or an alternative model, which is also explicitly featured in the N-P approach).
...
## Statistical decisions and long-term error control {#ch-03-04-hypothesis-significance-errors}
...
The significance of a test result, as a categorical measure, can then be further interpreted as a trigger for decision making. Commonly, a significant test result is interpreted as the signal to reject the null hypothesis, i.e., to speak and act as if it was false. 
...
2. Suppose that we have reason to believe that a coin is biased to land heads. A hypothesis test should shed light on this belief. We toss the coin $N = 10$ times and observe $k = 8$ heads. We set $\alpha = 0.05$.
...
<div class = "exercises">
**Exercise 10.2: $\chi^2$-test of independence**
...
<!-- exercise 3 -->
<div class = "exercises">
**Exercise 10.3: Two-sample $t$-test**
...
The degrees of freedom $d$ is given by the difference in free parameters, i.e., the number of parameters the nested model fixes to specific values, but which are free in the nesting model. 
...
## Relation to model checking {#ch-03-05-hypothesis-testing-3-model-checking}
