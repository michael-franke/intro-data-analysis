...
Bayesian ideas first been expressed in the 17th century (by Thomas Bayes) and have been given a solid mathematical treatment starting in the 18th century (by French mathematician Pierre-Simon Laplace).
...
As a result of eschewing subjective probability, the frequentist approach to statistical inference needs alternative methods for parameter estimation, inference and model comparison --- methods that do \emph{not} rely on (subjective) priors over parameters or models.
...
This assumption $\theta_{c} = 0.5$ is our *null hypothesis* $H_{0}$.
...
So, we assume the null hypothesis to be true for the same of argument.
...
The null-model assumes that $\theta_{c} = 0.5$ but also uses the obvious likelihood function, $\text{Binomial}(k, N, \theta_c = 0.5)$, stating how likely it would be to observe $k$ heads in $N$ tosses, for fixed $\theta_c$.
These probabilities are not subjective, but grounded in frequentist justification about limiting frequencies of events.
...
Fisher's approach calculates $p$-values as measures of evidence against a model, but does not necessarily prescribe what to do this this quantitative measure.
...
Which numbers make sense to you (given your current knowledge) and which do not? 

<div class="collapsibleSolution">
<button class="trigger">Solution</button>
<div class="content">

The output given first states what was computed, namely and *exact binomial test*.
...
**Case $\theta > 0.5$.** Let's now look at the case where we want to test whether the coin if biased towards heads $\theta > 0.5$. 
...
<div class = "exercises">
**Exercise 16.3: Significance & errors**
