...
The class of ANOVAs, which is short for "analysis of variance", comprises different models (similar to what we saw for the $t$-tests) to deal with the case of a metric dependent measurement, which is to be analyszed in terms of one or more categorical variable.
...
Nonetheless, it is good to have a basic understanding of ANOVA, as they are featured prominently in a lot of published research papers, whose findings are still relevant.
Also, in some areas of empirical science, ANOVAs are still commonly used.

Here we are just going to cover the most basic type of ANOVA, what is called a *one-way ANOVA*.
...
The frequentist model of a one-way ANOVA is shown in Figure \@ref(ch-05-01-ANOVA-onway-model).
...
Let's consider some concrete, but fictitious data:
...
We can calculate the one-way ANOVA explicitly as follows, following the calculations described in Figure XYZ:
