---
title: "Part 4: Models"
author: "Lecture: Introduction to Statistics"
...
\begin{quotation} 
	to obtain \emph{a large number of trials} from which emerged the relative frequency from an event. [@vallverdu2015, p.50] (emphasis is taken from the original)
\end{quotation}
...
where
\begin{itemize}
	\item $P(H|Data)$ is the conditional probability (\emph{posterior probability}) of the hypothesis H given a particular result (Data);  
	\item $P(Data|H)$ is the conditional probability (\emph{likelihood}) of a result(data) given the hypothesis H;
	\item $P(H)$ is the \emph{prior probability} of the hypothesis H;
	\item $P(Data)$ is the \emph{marginal (or average) likelihood}\footnote[1]{Sometimes also referred to as "Evidence"} of the data irrespective of the truth of any hypothesis.
\end{itemize}
...
where
\begin{itemize}
	\item $\theta$ is the probability of "head" for the coin flip;
	\item the bracket -[ ] indicates that we assume this parameter as unknown.
\end{itemize}
...
par(mfrow=c(2,2))
## theta=0.2
hist(rbinom(n=1e6,size=30,prob=0.2), xlab="k", main="Binomial(1e6,0.2)", 
     xlim = c(0,30), freq=FALSE)
## theta=0.5
hist(rbinom(n=1e6,size=30,prob=0.5), xlab="k", main="Binomial(1e6,0.5)", 
     xlim = c(0,30), freq=FALSE)
## theta=0.8
hist(rbinom(n=1e6,size=30,prob=0.8), xlab="k", main="Binomial(1e6,0.8)", 
     xlim = c(0,30), freq=FALSE)
```
...
dens(runif(n=1e6,min=0, max=1), ylim = c(0,1.5), 
     xlab=expression(theta), main="Uniform(0,1)")
## simulates a beta(1,1) distribution
dens(rbeta(n=1e6,shape1=1,shape2=1), ylim = c(0,1.5), 
