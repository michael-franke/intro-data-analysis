...
As we already know from the "probability"-lecture, for a coin flip the probability of each outcome can be described with the \emph{Bernoulli distribution}\, as there exist two discrete outcomes (head or tail) and a constant probability $\theta$:
...
The clearest difference, as already discussed, between frequentist and Bayesian methods is the incorporation of prior information in the Bayesian framework. The posterior distribution results by combining the likelihood (observed data) with the prior information.

When an uninformative (e.g. uniform) prior is used then the posterior is completely dependent on the data. The influence of the prior depends on their relative weighting. Considering for example the Beta distribution with the parameters a and b: $\beta \sim Beta(a,b)$. Assuming a coin flip experiment, the parameters of a Beta distribution can be interpreted as $a=$no. of heads and $b = $no. of tails, thus, $n=a+b$. Consequently, Beta(1,1) has a lower weight and thus influence on the posterior than Beta(10,10).

A lot of effort has been done in the area of "Objective Baysian data analysis" in order to develop "uninformativ prior distributions", because a lot of people feel uncomfortable using informative priors --- seeing them as biased or unscientific. On the contrary, most people interpret results based on their prior experiences and in this light, prior information is just a way to quantify this [@dobson2008].  

Important is that the definition of prior distributions make sense at every level in the hierarchy. Some further notes 
...
For example, consider again the coin flip experiment. The outcome of \emph{head} is influenced by the probability $\theta$. Further, $\theta$ is assumed to be distributed as Beta(1,1). A further hierarchical level can be created by defining a distribution on the parameters of the Beta prior. Such that the model can be assumed as follows:

<!-- ![Graphical notation hierarchical Beta-Binomial Modell - One group](https://github.com/michael-franke/intro-data-analysis/blob/master/chapters/images/binom-onelevel.png) -->

[Reparameterization here]

As such hierarchical models can be created with any degree of complexity: [can we use that?]

<!-- ![Graphical notation hierarchical Beta-Binomial Modell - One group](https://github.com/michael-franke/intro-data-analysis/blob/master/chapters/images/hierarchical-model.png) -->

## Exursos: Prior predictive distribution

Accoring to [@leeWagen2014]:

* the \emph{prior distribution} over parameters captures our initial assumptions or state of knowledge about the psychological variables they represent. 

* the \emph{prior predictive distribution} tells us what data to expect, given our model and our current state of knowledge. The prior predictive is a distribution over data, and gives the relative probability of different observable outcomes before any data have been seen.

The prior predictive distribution for the coin flip model (one group):

```{r}
# information on all parameters for which priors may be specified 
get_prior(formula = k|trials(x)~1, family=binomial(link = "logit"), data=coin.flip)
```
```{r}
# define prior according to the model
beta.prior <- set_prior("beta(1,1)",class = "Intercept")

# model and plot prior predictive 
prior.predictive <- brm(formula = k|trials(x)~1, family=binomial(link = "logit"), data=coin.flip, sample_prior="only", prior = beta.prior)
...
#### Excursos: Identically and independently distributed (\emph{iid})

The short model description $y_i\sim Normal(\mu, \sigma)$ incorporates often already an assumption about the distribution of distance-values: They are \emph{identically and independently ditributed}. Often the abbreviation \emph{iid} can be found for this assumption:
$$y_i\overset{\text{iid}}{\sim} Normal(\mu, \sigma).$$

The abbreviation \emph{iid} indicates that each value $y_i$ has the same probability function, independent of the other $y$ values and using the same parameters [@mcelreath2015]. This is hardly ever true. For example, thinking about the cars in the current example data set. Some cars may be of different types or even the same type but different batches. But the question is: Is this underlying dependency relevant for the model? If yes, this information has to be added in the model (e.g. in form of a hierachical model). Janyes states it as follows: \begin{quote} "*The onus is always on the user to make sure that all information, which his common sense tells him is relevant to the problem, is actually incorporated into the equations, (...).*"\end{quote}[@jaynes2003,p.339]. But if one do not know any relevant underlying relationships the most conservative distribution to use is \emph{iid}. Note, that the stated assumptions define how the model represents a problem and not how the world should be understood. For example, there might exist underlying correlations but on the overall distribution there influence tends towards zero. In such cases it remains usefull to assume iid [@mcelreath2015].
