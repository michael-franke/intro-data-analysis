...
$$y_i\overset{\text{iid}}{\sim} Normal(\hat{y}, \sigma).$$
...
In the third step, a Bayesian perspective is taken the **prior knowlege** (before seeing the data) has to be defined. The parameters of the current model are the predicted value $\hat{y}$ and the standard deviation $\sigma$. For the parameter $\hat{y}$ a normal distribution can be assumend with parameters that reflect the estimated values from the sample.
