...
To calculate the mean of a large vector, R has a built-in functioni `mean`, which we have in fact used frequently before. Let's use it to calculate the mean of the variable `average_price` for different types of avocados:
...
**Excursion** It is also possible to conceptualize the arithmetic mean as the expected value when sampling from the observed data. This is useful for linking the mean to the expected value of a random variable, a concept we will introduce in Chapter \@ref(Chap-03-01-probability). Suppose you have gathered the data $\vec{x} = \langle 0, 3, 6, 7\rangle$. Now you will sample from this data. What value do you expect to gain from one sample? -- What does that even mean?
  
Suppose that some joker from around town invites you for a game. The game goes like this. The joker puts a ball in an urn, one for each data observation.  You pay the joker a certain amount of money to be allowed to draw one ball from the urn. The balls are indistinguishable and the process of drawing is entirely fair. You receive the number corresponding the ball you drew paid out in silver coints. 

How many silver coins would you maximally pay to play one round? Well, of course, no more than four (unless you value gaming on top of silver)! This is because every ball has a chance of $0.25$ of being drawn. So you can expect to earn 0 silver with a 25% chance, 3 with a 25% chance, 6 with a 25% chance and 7 with a 25% chance. In this sense, the mean is the expected value of sampling once from the observed data. (And if this is not helpful for you right now, maybe you reread it later or you forget about it completely. -- End of excursion.)
```

#### The median

If $\vec{x} = \langle x_1, \dots , x_n \rangle$ is an ordered vector of $n$ data observations from an at least ordinal measure such that for all $1 \le i < n$ we have $x_i \le x_{i+1}$, the **median** is a value $x_i$ such that the number of data observations that are bigger or equal to $x_i$ and the number of data observations that are smaller or equal to $x_i$ are equal. Notice that this definition may yield no unique median. In that case different alternative stragies are used, depending on the data type at hand (ordinal or metric). (See also the example below.) The media corresponds to the 50% quartile.
...
There is no built-in function in R to return the mode of a (suitable) vector.
...
Bravais-Pearson correlation is covariance standardized by standard deviations:
...
```{r chap-02-03-avocado-scatter, echo = F, fig.cap="Scatter plot of avocado prices, plotted agains (logarithms of) the total amount sold. The black line is a linear regression line indicating the (negative) correlation between these measures (more in this later)."}
avocado_data %>% 
