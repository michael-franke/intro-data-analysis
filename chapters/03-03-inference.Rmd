...
Based on the notation above, which of the following statements you think is true?

a. prior always influence the posterior more, than the likelihood
...
d. likelihood always influence the posterior more, than the prior 
...
Can you think of a situation, when MLE and MAP output are the same? HINT: Think which prior eliminates the difference between them!

<div class="collapsibleSolution">
<button class="trigger">Solution</button>
<div class="content">

MLE is a special case of MAP with a uniform the prior.
...
Suppose Jones flipped a coin with unknown bias 30 times. She observed 20 heads. She updates her beliefs rationally with Bayes rule. Her posterior beliefs have the form of beta distribution with parameters a = 25, b = 15. What distribution and what parameter values of that distribution capture Jones' prior beliefs before updating her beliefs with this data?
...
Below is a shuffled list of steps that occur in the MH algorithm. Bring them in the right order!
...
e. choose to accept or reject the new proposal concerning the computed proportion
...
Next, we need to write the actual model. Notice that Stan code is strictly regimented to be divided into different blocks, so that Stan knows what is data, what are parameters and what consistitutes the actual model (prior and likelihood). Stan also wants to know the type of its variables (and the ranges of values these can take on).
...
The object returned from this call to Stan is a special model fit object. If we just print it, we get interesing information about the estimated parameters:
