...
- **accept** the point-valued hypothesis iff $[\theta^*_i - \epsilon; \theta^*_i + \epsilon]$ is contained entirely in $[l;u]$;
- **reject** the point-valued hypothesis iff $[\theta^*_i - \epsilon; \theta^*_i + \epsilon]$ and $[l;u]$ have no overlap; and
- **withhold judgement** otherwise.

Consider the 25/7 example, where the point-valued hypothesis of interest is $\theta^* = 0.5$ (testing the coin for fairness) and the ROPE is $[0.49; 0.51]$ ($\epsilon = 0.1$, arbitrarily set here). The point- and interval-estimates for Bayesian and frequentist approaches are as follows:
...
Figure \@ref(fig:ch-03-03-estimation-ROPE-24-7) shows these estimates next to the ROPE. We see that the Bayesian 95% credible interval has no overlap with the ROPE, so that we would *reject* the null-hypothesis of $\theta^* = 0.5$ by the ROPE+estimation logic of statistical decision making. If we were to illegitimately (!) apply this approach to frequentist confidence intervals, we would *accept* this point-valued hypotheses instead. However, as stressed above, this is *not* an accepted move in frequentist statistics. The frequenists would *not* consider a ROPE, or equivalently set $\epsilon = 0$ in each case. The frequentist would also not accept the point-valued hypothesis in case the critical value is included in the confidence interval. It would then merely withhold judgement, i.e., *not reject* the point hypothesis. (More on this in Chapter \@ref(ch-03-05-hypothesis-testing).)
...
estimates_24_7 %>% 
  ...
  geom_rect(
    aes(xmin = 0, xmax = 3, ymin = 0.49, ymax = 0.51), 
    fill = "firebrick", 
...
- [greta](https://greta-stats.org): R-only probabilistic modeling package, based on Python and tensoflow
...
We data from the [King of France example](app-93-data-sets-king-of-france), where we are interested in the number $k = 109$ of "true" responses to sentences with a false presuppsition over all $N = 311$ relevant observations. The research question of interest is whether the idea that the parameter $\theta$ which models to overal disposition to answer "true" for these sentences could plausibly be fixed to $\theta = 0.5$. We declare the relevant counts using the `greta` function `as_data`. In this way all subsequent calculations with `k` and `N` will make it clear that we are defining `greta`-objects as part of a (so far) static model definition.
...
Using the (controversial) metod of inspecting posterior estimates, we would conclude that $\theta = 0$ is not an *a posteriori* credible value for the inclination to judge the truth of sentences with a false presupposition. 
...
We will use the [Mental Chronometry](app-93-data-sets-mental-chronometry) data to compare the reaction times in the "go/No-go" condition to the reaction times in the "discrimination" condition. First we read in the data and get some handy summary statistics from the data:
