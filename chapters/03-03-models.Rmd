...
### Example: a single draw from an urn
...
### Example: avocado prices by type
...
Our model assumes that the data observations in `average_price` are samples from a normal distribution, whose mean and standard deviation are free paramters, which vary as a function of `type`. So, if $\vec{y}$ is the vector `average_price`, so that $y_i \in \mathbb{R}^+$ is the average price observed in row $i$, and if $\vec{x}$ is an indicator variable such that $x_i \in \{ 1, 0\}$ is the entry for the type of avocado in line $i$ where 1 represents conventionally grown and 0 represents organically grown, the likelihood function can be written as:

$$ P_M(\vec{y} \mid \vec{x}, \mu_c, \sigma_c, \mu_o, \sigma_o) = 
\prod_{i = 1}^k x_i \ \text{Normal}(y_i, \mu_c, \sigma_c) \ + \ (1- x_i) \  \text{Normal}(y_i, \mu_o, \sigma_o)  $$

The usual notation for this is more concise (see also Section \@ref(Chap-03-03-models-representation)), by identifying the free parameters as a vector $\theta = \langle \mu_c, \sigma_c, \mu_o, \sigma_o \rangle$, suppressing the independent variables, and using the `~` symbol in a manner so that "LHS ~ RHS" means that LHS is a sample from the distribution on the RHS.

$$ y_i \sim \begin{cases} \text{Normal}(y_i, \mu_c, \sigma_c) & \text{if} $x_i = 1$ \\ 
\text{Normal}(y_i, \mu_0, \sigma_0) & \text{otherwise} \\ \end{cases} $$ 
...
- free parameter
  - how free is a "free"? - not all parameters are equally powerful
- what to do with a model (3 pillars of DA):
  - inference
  - prediction
  - comparison
...
## Examples {#Chap-03-03-models-examples}
