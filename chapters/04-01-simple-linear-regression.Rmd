...
Concretely, we are interested in whether knowing a city's unemployment rate (stored in variable `unemployment`) helps make a better prediction for that city's murder rate (stored in variable `murder_rate`).
...
Suppose we know the vector of all observed murder rates but we don't know which murder rate belongs to which city.
We are given a city to guess its murder rate.
What's a good guess?

Actually, what is the best guess depends on what we want to do with this guess (the utility function of a decision problem).
For now, let's just assume that we have a measure of **prediction error** which we would like to minimize. 
A common measure of **prediction error** uses intuitions about geometric distance and is defined in terms of the **total sum of squares**, where $y$ is the $n$-dimensional vector of observed murder rates and $y'$ is the prediction we make:

$$
\text{TSS}(y') = \sum_{i=1}^n (y_i - y')^2
$$

This measure of prediction error is what underlies the ordinary least squares approach to simple regression.

It turns out that the **best prediction** we can make, i.e., the vector $\hat{y} = \arg \min TSS(y')$ for which TSS is minimized, is the mean $\hat{y}$ of the orginal predictions.
