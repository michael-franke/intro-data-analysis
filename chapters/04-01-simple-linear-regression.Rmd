...
If we knew all observed murder rates and if were to predict the murder rate of a random city so that we had no further information about that city, our best guess would be the mean of the observed values, because this is what minimizes (on average) the distance to the observed murder rates. 
...
The mean distance could be captured in terms of the **sum of squares** like this, where $y$ is the $n$-placed vector observed murder rates and $\bar{y}$ is its mean:
...
We can again quantify our prediction error in terms of the sum of squares like we did before. Here is how:
...
predicted_y <- 2 * x + 4
n <- length(y)
tss_guesswork <- sum((y - predicted_y)^2)
tss_guesswork
```

Compared to the previous prediction, which was based on the mean of $y$ only, this linear function reduces the prediction error (measured here geometrically in terms of a sum of squares).
...
We consider the simple linear relation (where the bar on top of vector $y$ symbolizes that this is a vector of predictions): 

$$ \bar{y}_i = \beta_0 + \beta_1 x_{1i} + \dots + \beta_k x_{ki}$$
...
Based on the predictions of a parameter vector $\langle \hat{\beta}_0, \hat{\beta}_1, \dots, \hat{\beta}_k\rangle$, we consider the total sum of squares as a measure of prediction error:

$$\text{TSS}_{\langle \hat{\beta}_0, \hat{\beta}_1, \dots, \hat{\beta}_k\rangle} = \sum_{i = 1}^k (y_i - \bar{y}_i)^2 $$

We would like to find the best parameter values (denoted traditionally by a hat on the parameter's variable: $\hat{\beta}_i$) in the sense of minimizing the total sum of squares:

$$
\langle \hat{\beta}_0, \hat{\beta}_1, \dots  , \hat{\beta}_k\rangle = \arg \min_{\langle \beta_0, \beta_1, \dots, \beta_k\rangle} \text{TSS}_{\langle \hat{\beta}_0, \hat{\beta}_1, \dots, \hat{\beta}_k\rangle}
$$
...
The next section will explain how we find these best-fitting parameter values.
...
get_tss = function(y, x, beta_0, beta_1) {
  yPred = beta_0 + x * beta_1
  sum((y-yPred)^2) 
}

# finding best-fitting values for TSS
fit_tss = optim(par = c(0, 1), 
  fn = function(par) {
    get_tss(y, x, par[1], par[2])
  }
)

message(
  "Best fitting parameter values:",
  "\n\tIntercept: ", fit_tss$par[1] %>% signif(5),
  "\n\tSlope: ", fit_tss$par[2] %>%  signif(5),
  "\nTSS for best fit: ", fit_tss$value %>% signif(5)
)
```

### Fitting OLS regression lines with `lm`

R also has a built-in function `lm` which fits (simple) linear regression models via TSS minimization. Here is how you call this function for the running example:
