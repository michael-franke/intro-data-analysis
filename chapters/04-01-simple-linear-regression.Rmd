...
This section introduces ordinary least-squares linear regression. The main idea is that we look for the best-fitting line in a (multi-dimensional) clouod of points, where "best-fitting" is defined in terms of a geometrical measure of distance (squared prediction error).
...
The mean distance could be captured in terms of the **total sum of squares** like this, where $y$ is the $n$-placed vector of observed murder rates and $\bar{y}$ is its mean:
...
The availability of these elegant mathematical solutions for OLS-regression explains why the computation of best-fitting regression coefficients with a built-in function like `lm` is lightning fast: it does not rely on optimization with `optim`, sampling methods or similar computational approaches.
