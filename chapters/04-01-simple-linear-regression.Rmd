...
<button class="trigger">Solution</button>
<div class="content">
The naive model have only $\beta_0$ as its coefficient with it being the mean of $y$ values, i.e. $\hat{\beta}_0$ = $\bar{y}$.
</div>
</div>
</div>
...
Let's assume following the MLE approach, we obtained $\beta_0 = 1$, $\beta_1 = 2$ and $\sigma = 0.5$. For $x_i = 0$, which $y_i$ value will maximize the likelihood?
...
Compare the output of `fit_lm` on the last page and `fit_glm` on this page. Why are the estimates of coefficients same?
...
Here is a plot of the posterior: [TODO: redo with Stan]
...
<button class="trigger">Solution</button>
<div class="content">
The estimate will have a value that is bigger than the current one, especially if the number of data points are small.
</div>
</div>
</div>
...
Where can one finde $p$-values in the above output? What does it mean?

<div class="collapsibleSolution">
<button class="trigger">Solution</button>
<div class="content">
$p$-values can be found under the column 'Pr(>|t|)'. Low $p$-values are evidences against the null hypothesis, which in this case assumes that values of coefficients are $0$. In other word, we have strong evidence to believe that the mean difference between two groups are not 0.
</div>
</div>
</div>
